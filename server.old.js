...
// ---- Job status tracking (in-memory cache) ----
// We'll persist to Postgres tasks table when pgPool is available
const jobs = new Map() // task_id -> { status, file_id, public_url, timestamp, error }
...
// CORS with allowlist (supports multiple domains from env)
...
if (origins.length > 0) {
  app.use(
    cors({
      origin: (origin, cb) =>
        !origin || origins.includes(origin) ? cb(null, true) : cb(new Error('Not allowed by CORS')),
      credentials: false,
    })
  )
...
// Rate limiting for demo endpoints (more lenient)
const demoLimiter = rateLimit({
  windowMs: 60_000,
  max: 10, // 10 demos per minute
...
╠═══════════════════════════════════════════════════════════╣
║  Security Features: 🔒                                    ║
║  ✅ CORS allowlist (${origins.length > 0 ? origins.length + ' origin(s)' : 'unrestricted'})${' '.repeat(Math.max(0, 27 - (origins.length > 0 ? (origins.length + ' origin(s)').length : 'unrestricted'.length)))}║
